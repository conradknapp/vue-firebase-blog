          ...
          <figcaption v-if="!reading || $device.phone" class="preview__details">
            ...
            <div class="preview__meta">
              <time class="preview__published">
                {{ prettyDate(post.meta.published) }}
              </time>

              <router-link class="preview__author"
                :to="`/by/${kebabify(post.meta.author)}`"
                @click.native="scrollTo(0, 220, scrollDelay)">
                {{ post.meta.author }}
              </router-link>
            </div>
...
export default {
  ...
  created() {
    ...
    //   let posts = this.$store.getters.loadedPosts
    //   resolve(posts)
    // })
    // p.then(posts => {
    //   console.log(posts)
    //   if (!Object.keys(this.filters).length && !this.$store.getters.loading) {
